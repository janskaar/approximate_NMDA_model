...
outfile = os.path.join("decision_making_sparse_adjusted", f"runner_{runner_id}.h5")
...
def run_sim(s1, s2, seed=123):
    ...
    conn_spec = {"rule": "fixed_indegree", "indegree": int(NE * f * epsilon * sel_factor), "allow_multapses": False}
    ...
    conn_spec = {"rule": "fixed_indegree", "indegree": int(NI * epsilon * inh_factor), "allow_multapses": False}
    nest.Connect(
    ...
    val = s1 * NE * f * w_plus * epsilon * sel_factor + s2 * NE * f * w_minus * epsilon
    selective_pop1.set({"s_NMDA_post_current": val * g_NMDA_ex / epop_params["tau_decay_NMDA"],
                        "s_NMDA_pre_clamp": True,
                        "s_NMDA_pre_value": s1})


    val = s1 * NE * f * w_minus * epsilon + s2 * NE * f * w_plus * epsilon * sel_factor
...
inh_factors = np.linspace(0.2,0.5,13) / epsilon
sel_factors = np.linspace(0.2,1.0,31) / epsilon

i = 0
for inh_factor in inh_factors:
    for sel_factor in sel_factors:
        i += 1
        ...
        with h5py.File(outfile, "a") as f:
            ...
            inner_group.attrs["sel_factor"] = sel_factor
            inner_group.attrs["inh_factor"] = inh_factor
